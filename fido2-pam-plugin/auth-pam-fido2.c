...
/*
 * OpenVPN plugin module to do PAM and FIDO2 Two Factor Authentication
 * using a split privilege model.
 * Uses a supporting python script, auth-pam-fido2.py, for FIDO2 functionality
...
#define FIDO2_SCRIPT_PATH "/usr/share/openvpn/pam-fido2/auth-fido2.py"
...
openvpn_plugin_func_v2(openvpn_plugin_handle_t handle, const int type, const char *argv[], const char *envp[], void *per_client_context, struct openvpn_plugin_string_list **return_list)
{
    ...
    if (type == OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY && context->foreground_fd >= 0)
    {
        ...
        const char *script_path = get_env("fido2_script_path", envp) ? get_env("fido2_script_path", envp) : FIDO2_SCRIPT_PATH;
        const char *fido2_origin = get_env("fido2_origin", envp);
        const char *fido2_name = get_env("fido2_name", envp);
        ...
        if (username && strlen(username) > 0 && password)
        {
            if (send_control(context->foreground_fd, COMMAND_VERIFY) == -1
                ...
                || send_string(context->foreground_fd, fido2_origin) == -1
                || send_string(context->foreground_fd, fido2_name) == -1)
            {
        ...
        switch (command)
        {
            case COMMAND_VERIFY:
                if (recv_string(fd, up.username, sizeof(up.username)) == -1
                    ...
                    || recv_string(fd, up.fido2_origin, sizeof(up.fido2_origin)) == -1
                    || recv_string(fd, up.fido2_name, sizeof(up.fido2_name)) == -1)
                {
...
fido2_auth_verify(const struct user_pass *up, bool hasChallengeResponse, char **client_reason)
{
    ...
    if (pid < 0) {
	    ...
	    setenv("username", up->username, 1);
        if (hasChallengeResponse) {
	        setenv("password", up->password, 1);
        }
	    setenv("fido2_origin", up->fido2_origin, 1);
	    setenv("fido2_name", up->fido2_name, 1);
        execvp(argv[0], argv);
        exit(1);
    }
    close(pipefd[1]);
    ...
    fprintf(stderr, "AUTH-PAM: BACKGROUND: FIDO2 Script Failed Unexpectedly\n");
    //Error
    return OPENVPN_PLUGIN_FUNC_ERROR;
}
